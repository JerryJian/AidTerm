...
| 层 | 技术 |
|----|------|
| 桌面框架 | Tauri 2.x (Rust) + Electron（双后端，`src/api` 自动检测） |
| 前端 | Vue 3 + TypeScript + Vite |
| 状态管理 | Pinia |
| 路由 | Vue Router |
| 终端渲染 | xterm.js + addons |
| 样式 | 原生 CSS / flex（UnoCSS/Tailwind 未用） |
| 后端语言 | Rust / TypeScript（Electron main） |
| SSH | `russh`（Rust）/ `ssh2`（Electron） |
| SFTP | `russh-sftp` / `ssh2-sftp-client` |
| Serial | `tokio-serial` / `serialport` |
| PTY | `portable-pty` / `node-pty` |
| 代理 | `tokio-socks` |
| AI | `reqwest` + `async-openai` + `ollama-rs`（OpenAI 兼容） |
| 密钥 | `russh::keys`，known_hosts 本地文件 |
...
│   ├── api/                # tauri.ts / electron.ts / index.ts 统一适配层
│   ├── components/
...
4. **所有 IPC 消息需有明确的类型定义**，前后端共享 `types`
5. **断开/错误路径必须发 `session-status disconnected`**，保证前端覆盖层可触发（已统一处理）
