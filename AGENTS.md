...
## 项目概述
使用 **Tauri 2.x** + **Vue 3 (Composition API + TypeScript)** + **Rust** 后端，构建跨平台终端模拟器。
...
| 1.7 | 无限回滚 | xterm scrollback 可配置（1000–1000000 行）✅ |
...
| 2.5 | 连接复用 + 重连 | 手动重连：断开覆盖层支持 Enter / 按钮 / 右键菜单 ✅（自动重试待做） |
