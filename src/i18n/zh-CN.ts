export default {
  app: {
    title: 'AidTerm',
    lock: '锁屏',
    unlock: '解锁',
    quit: '退出',
  },
  tab: {
    new_tab: '新建标签',
    close_tab: '关闭标签',
    close_others: '关闭其他',
    close_right: '关闭右侧',
    rename: '重命名',
    export_text: '导出文本',
    split_horizontal: '左右拆分',
    split_vertical: '上下拆分',
    new_ssh: '新建 SSH',
    local: '本地',
    ssh: 'SSH',
    telnet: 'Telnet',
    serial: '串口',
    toggle_sidebar: '切换侧栏',
    tools: '工具',
  },
  titlebar: {
    minimize: '最小化',
    maximize: '最大化',
    restore: '恢复',
    close: '关闭',
    inspect: '检查 (DevTools)',
  },
  context_menu: {
    copy: '复制',
    paste: '粘贴',
    select_all: '全选',
    search: '搜索',
    clear: '清除',
    new_tab: '新建标签',
    new_ssh: '新建 SSH',
    close_tab: '关闭标签',
    ask_ai: '🤖 AI 解释选中内容',
    reconnect: '重新连接',
  },
  quick_connect: {
    title: '快速连接',
    placeholder: 'ssh user@host 或 telnet host:port',
    ssh_btn: 'SSH 详细',
    telnet_btn: 'Telnet',
  },
  ssh_dialog: {
    title: 'SSH 连接',
    host: '主机',
    port: '端口',
    username: '用户名',
    password: '密码',
    private_key: '私钥文件',
    browse: '浏览',
    auth_type: '认证方式',
    password_auth: '密码',
    key_auth: '密钥',
    proxy: '代理',
    none: '无',
    connect: '连接',
    cancel: '取消',
    agent_forwarding: 'SSH Agent 转发',
    x11_forwarding: 'X11 转发',
    save_session: '保存会话',
  },
  telnet_dialog: {
    title: 'Telnet 连接',
    host: '主机',
    port: '端口',
    connect: '连接',
    cancel: '取消',
  },
  serial_dialog: {
    title: '串口连接',
    port: '端口',
    select_port: '选择串口...',
    baud_rate: '波特率',
    data_bits: '数据位',
    stop_bits: '停止位',
    parity: '校验位',
    flow_control: '流控制',
    connect: '连接',
    cancel: '取消',
  },
  wsl_dialog: {
    title: 'WSL 连接',
    distro: '发行版',
    distro_placeholder: '选择发行版（留空使用默认发行版）',
    working_dir: '工作目录（可选）',
    working_dir_placeholder: '如 /home/user；留空继承父进程目录',
    refresh: '刷新',
    connect: '连接',
    cancel: '取消',
  },
  adb_dialog: {
    title: 'Android 设备 (ADB)',
    refresh: '刷新',
    no_devices: '未发现设备，请确认设备已开启 USB 调试。',
    caveat: '提示：已被其他 ADB 服务(端口 5037)占用的设备不会在此显示。首次连接时请在手机上确认 RSA 指纹授权。',
    occupied: '已被系统 ADB 服务 (5037) 占用，请先停止占用后重试：',
    connect: '连接 Shell',
    cancel: '取消',
    missing_title: '未检测到 adb',
    missing_desc: '当前安装包未内置 adb（Linux arm64 等平台官方没有预编译的 platform-tools），系统 PATH 中也未找到 adb。',
    missing_install_title: '安装 Android platform-tools：',
    missing_install_linux: '# Debian / Ubuntu\nsudo apt install adb\n# Fedora\nsudo dnf install android-tools\n# Arch\nsudo pacman -S android-tools',
    missing_install_mac: 'brew install --cask android-platform-tools',
    missing_install_win: '# 下载并解压 platform-tools 后，把目录加入 PATH\nhttps://developer.android.com/tools/releases/platform-tools',
    missing_env: '或将 adb 可执行文件的完整路径填入环境变量 AIDTERM_ADB，重启应用后即可使用。',
  },
  session_panel: {
    title: '会话管理',
    groups: '分组',
    all: '全部',
    search: '搜索会话...',
    connect: '连接',
    delete: '删除',
    recent: '最近使用',
    no_sessions: '暂无会话',
    ungrouped: '未分组',
    empty_group: '(空)',
    show: '显示',
    hide: '隐藏',
  },
  shell: {
    cmd: '命令提示符',
    powershell: 'Windows PowerShell',
    pwsh: 'PowerShell',
    wsl: 'WSL',
    bash: 'Bash',
    zsh: 'Zsh',
    sh: 'Sh',
    fish: 'Fish',
  },
  menu: {
    settings: '设置',
    lock: '锁屏',
    batch_mode: '批量模式',
    views: '视图',
    sessions: '会话管理',
    quick_connect: '快速连接',
    new_ssh: '新建 SSH...',
    local_shell: '本地终端',
    wsl: 'WSL',
    wsl_advanced: 'WSL (高级)...',
    saved_sessions: '已保存会话',
    remote_connection: '远程连接',
    ssh: 'SSH...',
    telnet: 'Telnet...',
    serial: '串口...',
    adb: 'Android (ADB)...',
  },
  tool_panel: {
    ai: 'AI 助手',
    history: '命令历史',
    sftp: '文件管理',
    adb_files: '设备文件',
    tunnel: '隧道',
    proxy: '代理',
    snippet: '快捷命令',
    trigger: '触发器',
    key: '密钥',
    knownHosts: '主机',
    cast: '投屏',
    monitor: '监控',
  },
  cast_panel: {
    start: '开始',
    stop: '停止',
    starting: '启动中...',
    waiting: '点击开始以镜像设备屏幕',
    not_supported: '当前环境不支持投屏',
    no_device: '未选择 ADB 设备',
    failed: '投屏失败：{msg}',
    disconnected: '投屏已结束',
    hint_tap: '单击=点按，拖动=滑动，滚轮=滚动',
    power: '电源键',
    volume_up: '音量加',
    volume_down: '音量减',
    nav_back: '返回',
    nav_home: '主页',
    nav_recent: '最近任务',
    reconnecting: '正在重连...',
    disconnect: '断开连接',
  },
  monitor_panel: {
    title: '系统监控',
    cpu: 'CPU',
    memory: '内存',
    swap: '交换分区',
    disk: '磁盘',
    network: '网络',
    gpu: '显卡',
    usage: '占用率',
    vram: '显存',
    cpu_history: 'CPU 趋势',
    used: '已用',
    free: '空闲',
    rx: '下行',
    tx: '上行',
    load: '负载',
    total: '总计',
    unavailable: '无法获取远程监控数据',
    no_session: '无可用会话',
    core: '核',
    refresh: '刷新',
  },
  status: {
    tabs_count: '{count} 个标签页',
    local: '本地',
    connected: '已连接',
    connecting: '连接中',
    disconnected: '已断开',
    type: '类型',
    host: '主机',
    port: '端口',
    user: '用户',
    auth: '认证',
    password: '密码',
    key: '密钥',
    hostname: '主机名',
    kernel: '内核',
    shell: 'Shell',
    agent: 'Agent 转发',
    enabled: '是',
    status: '状态',
  },
  session_dialog: {
    title_new: '新建会话',
    title_edit: '编辑会话',
    name: '名称',
    type: '类型',
    host: '主机',
    port: '端口',
    username: '用户名',
    password: '密码',
    remember_password: '记住密码',
    group: '分组',
    no_group: '无分组',
    new_group: '新建分组...',
    new_group_placeholder: '输入新分组名称',
    cancel: '取消',
    save: '保存',
    create: '创建',
    local_terminal: '本地终端',
    command: '可执行文件',
    command_placeholder: '如 cmd.exe、/bin/bash',
    working_dir: '工作目录',
    working_dir_placeholder: '留空则继承父进程',
    icon: '图标',
  },
  sftp: {
    panel_title: '文件管理',
    connect: '连接',
    disconnect: '断开',
    go_up: '上一级',
    folder_name: '文件夹名',
    ok: '确定',
    new_file: '新建文件',
    actions: '操作',
    empty_directory: '空目录',
    title: '文件管理 - {host}',
    local: '本地',
    remote: '远程',
    upload: '上传',
    download: '下载',
    rename: '重命名',
    delete: '删除',
    mkdir: '新建文件夹',
    refresh: '刷新',
    name: '名称',
    size: '大小',
    modified: '修改时间',
    permissions: '权限',
    bytes: 'B',
    kb: 'KB',
    mb: 'MB',
    gb: 'GB',
    dir: '文件夹',
    file: '文件',
    close: '关闭',
    edit: '编辑',
    no_files: '暂无文件',
    connecting: '连接中...',
    disconnected: '已断开',
    drop_to_upload: '拖拽文件到此处上传',
    uploading: '上传中...',
    downloading: '下载中...',
    upload_done: '完成',
    upload_failed: '失败',
    cancelled: '已取消',
    confirm_delete: '确认删除 {name}？',
    rename_title: '重命名',
    create_title: '创建{0}',
    enter_name: '输入名称',
    owner: '所有者',
    group: '组',
    other: '其他',
  },
  adb_file: {
    not_connected: '设备未连接。请先在设备列表连接一个 adb 会话，再打开文件面板。',
  },
  file_fs: {
    local_ready: '本地文件系统，打开即浏览本机磁盘。',
  },
  tunnel: {
    title: '端口转发',
    create: '新建隧道',
    new_btn: '新建',
    cancel_btn: '取消',
    ssh_server: 'SSH 服务器',
    authentication: '认证',
    password_optional: '密码（可选）',
    browse: '浏览',
    forward_type: '转发类型',
    local_label: '本地 (-L)',
    remote_label: '远程 (-R)',
    dynamic_label: '动态 (-D, SOCKS5)',
    bind_address: '绑定地址',
    target: '目标地址',
    create_forward: '创建转发',
    private_key_path: '私钥路径（可选）',
    local: '本地转发',
    remote: '远程转发',
    dynamic: '动态转发',
    name: '名称',
    type: '类型',
    src_host: '源主机',
    src_port: '源端口',
    dst_host: '目标主机',
    dst_port: '目标端口',
    status: '状态',
    actions: '操作',
    start: '启动',
    stop: '停止',
    remove: '删除',
    starting: '启动中',
    running: '运行中',
    stopped: '已停止',
    error: '错误',
    add: '添加',
    cancel: '取消',
    no_tunnels: '暂无隧道',
    using_session: '使用当前 SSH 会话：{0}',
  },
  proxy: {
    title: '代理配置',
    add: '添加代理',
    name: '名称',
    type: '类型',
    host: '主机',
    port: '端口',
    username: '用户名',
    password: '密码',
    http: 'HTTP',
    socks5: 'SOCKS5',
    jump_host: '跳板机',
    save: '保存',
    delete: '删除',
    cancel: '取消',
    private_key_path: '私钥路径 (可选)',
    select: '选择',
    no_proxies: '暂无代理',
  },
  snippet: {
    title: '快捷命令',
    add: '添加命令',
    name: '名称',
    command: '命令',
    variables: '变量',
    send: '发送',
    edit: '编辑',
    delete: '删除',
    close: '关闭',
    no_snippets: '暂无快捷命令',
    variable_hint: '输入变量值',
    variable_usage_hint: '使用双花括号包裹变量名定义变量，发送时会提示输入',
    cancel: '取消',
    confirm: '确定',
  },
  trigger: {
    title: '触发器',
    add: '添加触发器',
    pattern: '匹配模式',
    response: '响应命令',
    cooldown: '冷却(秒)',
    enabled: '启用',
    disabled: '禁用',
    edit: '编辑',
    delete: '删除',
    close: '关闭',
    no_triggers: '暂无触发器',
    save: '保存',
    cancel: '取消',
  },
  history: {
    title: '命令历史',
    search: '搜索命令...',
    clear: '清空',
    execute: '执行',
    delete: '删除',
    empty: '暂无命令历史，终端中执行的命令会自动记录',
    no_match: '无匹配的命令',
  },
  batch: {
    title: '批量输入',
    placeholder: '批量输入命令到选中终端...',
    select_all: '全选',
  },
  editor: {
    title: '编辑文件',
    save: '保存',
    saving: '保存中...',
    saved: '已保存',
    loading: '加载中...',
    close: '关闭',
    error: '错误',
    cut: '剪切',
    copy: '复制',
    paste: '粘贴',
    select_all: '全选',
  },
  lock_screen: {
    title: '已锁定',
    placeholder: '输入密码解锁',
    unlock: '解锁',
    set_password: '设置锁屏密码',
    confirm_password: '确认密码',
    set: '设置',
    change_password: '修改密码',
    wrong_password: '密码错误',
    password_mismatch: '两次密码不一致',
    no_password: '请先设置锁屏密码',
    lock: '锁定',
    unlock_failed: '解锁失败',
  },
  settings: {
    title: '设置',
    language: '语言',
    language_zh: '中文',
    language_en: 'English',
    theme: '主题',
    dark: '暗色',
    light: '亮色',
    background: '背景图',
    background_image: '背景图片',
    background_opacity: '背景图透明度',
    select_image: '选择图片',
    clear_image: '清除图片',
    scrollback: '回滚行数',
    general: '常规',
    appearance: '外观',
    fullscreen: '全屏',
    exit_fullscreen: '退出全屏',
    guake_mode: 'Guake 模式',
    adb_auto_kill: '最后一个 ADB 标签关闭后停止 ADB 服务',
    adb_auto_kill_desc: '使用程序自带 adb 时运行在独立的 5038 端口服务；使用系统/外部 adb 时直接使用默认的 5037 服务。',
    shell_context_menu: '添加“在 AidTerm 中打开”右键菜单',
    shell_context_menu_desc: '添加到目录、目录空白处和桌面空白处的右键菜单。',
    path_environment: '将 aidterm 添加到环境变量 PATH',
    path_environment_desc: '允许在终端中直接使用 aidterm 命令；新开的终端生效。',
    single_instance: '单例模式',
    single_instance_desc: '只允许运行一个 AidTerm 实例，再次启动时聚焦已有窗口；更改后重启生效。',
    lock_screen: '启用锁屏',
    lock_screen_desc: '启用后标题栏显示锁屏按钮；锁定后需输入密码解锁。',
    lock_screen_password: '锁屏密码',
    lock_screen_set_password: '设置锁屏密码',
    lock_screen_change_password: '修改锁屏密码',
    lock_screen_new_password: '新密码',
    lock_screen_confirm_password: '确认新密码',
    lock_screen_current_password: '当前密码',
    lock_screen_password_hint: '尚未设置锁屏密码，请在启用后设置。',
    lock_screen_wrong_current: '当前密码不正确',
    lock_screen_saved: '锁屏密码已保存',
    ai_provider: '提供商',
    ai_api_config: 'API 配置',
    ai_status: '状态',
    ai_configured: '✅ 已配置',
    ai_not_configured: '❌ 未配置 (请输入 API Key)',
    ai_clear_history: '清除对话历史',
    ai_show: '显示',
    ai_hide: '隐藏',
  },
  notification: {
    disconnected: '连接已断开',
    reconnected: '连接已恢复',
    error: '错误',
  },
  keychain: {
    title: '密钥管理',
    generate: '生成',
    import: '导入',
    generate_keypair: '生成密钥对',
    ed25519: 'ED25519 (推荐)',
    key_name: '密钥名称',
    bits: '位数：',
    passphrase: '密码短语 (可选)',
    key_path: '私钥文件路径',
    browse: '浏览',
    loading: '加载中...',
    no_keys: '暂无密钥',
    copy_public: '复制公钥',
    copy_path: '复制私钥路径',
    delete: '删除',
    refresh: '刷新',
    known_hosts: '已知主机',
    known_hosts_empty: '~/.ssh/known_hosts 为空',
  },
  ai: {
    title: 'AI',
    sidebar_title: 'AI 助手',
    suggest_command: 'AI 建议执行命令',
    execute: '执行',
    modify: '修改',
    cancel: '取消',
    provider: '提供商',
    api_config: 'AI 配置',
    api_key: 'API Key',
    model: 'Model',
    base_url: 'Base URL',
    status: '状态',
    configured: '✅ 已配置',
    not_configured: '❌ 未配置 (请输入 API Key)',
    clear_history: '清除对话历史',
    select_model: '选择模型',
    explain_selection: '🤖 AI 解释选中内容',
    mode: '触发方式',
    mode_auto: '自动识别',
    mode_prefix: '前缀触发',
    mode_keybinding: '按键发送 (Ctrl+Enter)',
    prefix: '触发前缀',
    prefix_desc: '输入此字符开头的文本将触发 AI 应答，每字符均为独立触发前缀',
    input_placeholder: '输入问题... (Enter 发送)',
    cancel_conversation: '取消本次问答',
    thinking: '思考中...',
    executing: '执行中...',
    analyzing: 'AI 分析中...',
    output_truncated: '输出较长，已截断发送给 AI',
    config_placeholder: '请先配置 AI 服务',
    config_click: '点击此处打开配置',
    no_config: '请在设置 → AI 中配置 API Key 后使用 AI 助手',
    no_active_terminal: '暂无活动终端，请选择一个终端',
    reset: '重置对话',
    new_conversation: '新对话',
    dangerous_command: '危险命令',
    safe_command: '安全命令',
    auto_executing: '自动执行中...',
    auto_execute_done: '执行完成',
    auto_execute_section: '命令执行',
    auto_execute: '自动执行安全命令',
    auto_execute_desc: '开启后，AI 判断为安全的查询命令将自动执行，危险命令仍需确认',
    command_output: '命令输出',
    stop_waiting: '停止等待',
    copy: '复制',
    select_all: '全选',
    me: '我',
    aid: 'Aid',
  },
  common: {
    name: '名称',
    type: '类型',
    host: '主机',
    port: '端口',
    username: '用户名',
    password: '密码',
    cancel: '取消',
    save: '保存',
    delete: '删除',
    edit: '编辑',
    status: '状态',
    loading: '加载中...',
  },
  terminal: {
    connecting: '连接中...',
    disconnected: '连接已断开',
    reconnect: '重新连接',
    enter_to_reconnect: '按 Enter 键重新连接',
  },
  about: {
    title: '关于',
    description: '基于 Tauri 和 Vue 构建的跨平台终端模拟器，支持 SSH、Telnet、串口、SFTP 及 AI 智能助手。',
    github: 'GitHub',
    check_update: '检查更新',
    checking: '正在检查...',
    up_to_date: '当前已是最新版本 v{version}',
    update_available: '发现新版本 v{version}',
    published: '发布时间',
    installer: '安装包类型',
    download_install: '下载并安装',
    downloading: '正在下载',
    install_restart: '安装并重启',
    view_release: '查看发布详情',
    view_update: '查看更新详情',
  },
}
