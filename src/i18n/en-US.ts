export default {
  app: {
    title: 'AidTerm',
    lock: 'Lock',
    unlock: 'Unlock',
    quit: 'Quit',
  },
  tab: {
    new_tab: 'New Tab',
    close_tab: 'Close Tab',
    close_others: 'Close Others',
    close_right: 'Close to Right',
    rename: 'Rename',
    export_text: 'Export Text',
    split_horizontal: 'Split Left-Right',
    split_vertical: 'Split Top-Bottom',
    new_ssh: 'New SSH',
    local: 'Local',
    ssh: 'SSH',
    telnet: 'Telnet',
    serial: 'Serial',
    toggle_sidebar: 'Toggle Sidebar',
    tools: 'Tools',
  },
  titlebar: {
    minimize: 'Minimize',
    maximize: 'Maximize',
    restore: 'Restore',
    close: 'Close',
    inspect: 'Inspect (DevTools)',
  },
  context_menu: {
    copy: 'Copy',
    paste: 'Paste',
    select_all: 'Select All',
    search: 'Search',
    clear: 'Clear',
    new_tab: 'New Tab',
    new_ssh: 'New SSH',
    close_tab: 'Close Tab',
    ask_ai: '🤖 AI Explain Selection',
    reconnect: 'Reconnect',
  },
  quick_connect: {
    title: 'Quick Connect',
    placeholder: 'ssh user@host or telnet host:port',
    ssh_btn: 'SSH Detail',
    telnet_btn: 'Telnet',
  },
  ssh_dialog: {
    title: 'SSH Connection',
    host: 'Host',
    port: 'Port',
    username: 'Username',
    password: 'Password',
    private_key: 'Private Key',
    browse: 'Browse',
    auth_type: 'Auth Type',
    password_auth: 'Password',
    key_auth: 'Key',
    proxy: 'Proxy',
    none: 'None',
    connect: 'Connect',
    cancel: 'Cancel',
    agent_forwarding: 'SSH Agent Forwarding',
    x11_forwarding: 'X11 Forwarding',
    save_session: 'Save Session',
  },
  telnet_dialog: {
    title: 'Telnet Connection',
    host: 'Host',
    port: 'Port',
    connect: 'Connect',
    cancel: 'Cancel',
  },
  serial_dialog: {
    title: 'Serial Connection',
    port: 'Port',
    select_port: 'Select port...',
    baud_rate: 'Baud Rate',
    data_bits: 'Data Bits',
    stop_bits: 'Stop Bits',
    parity: 'Parity',
    flow_control: 'Flow Control',
    connect: 'Connect',
    cancel: 'Cancel',
  },
  wsl_dialog: {
    title: 'WSL Connection',
    distro: 'Distribution',
    distro_placeholder: 'Select distribution (leave empty for default)',
    working_dir: 'Working directory (optional)',
    working_dir_placeholder: 'e.g. /home/user; empty inherits the parent directory',
    refresh: 'Refresh',
    connect: 'Connect',
    cancel: 'Cancel',
  },
  adb_dialog: {
    title: 'Android Device (ADB)',
    refresh: 'Refresh',
    no_devices: 'No devices found. Make sure USB debugging is enabled on the device.',
    caveat: 'Note: devices already connected to another ADB server (port 5037) will not appear here. On first connection, accept the RSA fingerprint prompt on your phone.',
    occupied: 'Held by another ADB server (port 5037). Stop that server to use these devices here:',
    connect: 'Connect Shell',
    cancel: 'Cancel',
    missing_title: 'adb not found',
    missing_desc: 'This build ships no bundled adb (there is no official prebuilt platform-tools for Linux arm64), and no adb was found on the system PATH.',
    missing_install_title: 'Install Android platform-tools:',
    missing_install_linux: '# Debian / Ubuntu\nsudo apt install adb\n# Fedora\nsudo dnf install android-tools\n# Arch\nsudo pacman -S android-tools',
    missing_install_mac: 'brew install --cask android-platform-tools',
    missing_install_win: '# Download and unzip platform-tools, then add the folder to PATH\nhttps://developer.android.com/tools/releases/platform-tools',
    missing_env: 'Or set the AIDTERM_ADB environment variable to the full path of an adb executable and restart the app.',
  },
  session_panel: {
    title: 'Session Manager',
    groups: 'Groups',
    all: 'All',
    search: 'Search sessions...',
    connect: 'Connect',
    delete: 'Delete',
    recent: 'Recent',
    no_sessions: 'No sessions',
    ungrouped: 'Ungrouped',
    empty_group: '(empty)',
    show: 'Show',
    hide: 'Hide',
  },
  shell: {
    cmd: 'Command Prompt',
    powershell: 'Windows PowerShell',
    pwsh: 'PowerShell',
    wsl: 'WSL',
    bash: 'Bash',
    zsh: 'Zsh',
    sh: 'Sh',
    fish: 'Fish',
  },
  menu: {
    settings: 'Settings',
    lock: 'Lock',
    batch_mode: 'Batch Mode',
    views: 'Views',
    sessions: 'Sessions',
    quick_connect: 'Quick Connect',
    new_ssh: 'New SSH...',
    local_shell: 'Local Shell',
    wsl: 'WSL',
    wsl_advanced: 'WSL (Advanced)...',
    saved_sessions: 'Saved Sessions',
    remote_connection: 'Remote Connection',
    ssh: 'SSH...',
    telnet: 'Telnet...',
    serial: 'Serial...',
    adb: 'Android (ADB)...',
  },
  tool_panel: {
    ai: 'AI Assistant',
    history: 'Command History',
    sftp: 'Files',
    adb_files: 'Device Files',
    tunnel: 'Tunnel',
    proxy: 'Proxy',
    snippet: 'Snippets',
    trigger: 'Triggers',
    key: 'Keys',
    knownHosts: 'Hosts',
    cast: 'Cast',
    monitor: 'Monitor',
  },
  cast_panel: {
    start: 'Start',
    stop: 'Stop',
    starting: 'Starting...',
    waiting: 'Click Start to mirror the device screen',
    not_supported: 'Screen casting is not supported in this environment',
    no_device: 'No ADB device selected',
    failed: 'Cast failed: {msg}',
    disconnected: 'Cast ended',
    hint_tap: 'Click to tap, drag to swipe, wheel to scroll',
    power: 'Power',
    volume_up: 'Volume Up',
    volume_down: 'Volume Down',
    nav_back: 'Back',
    nav_home: 'Home',
    nav_recent: 'Recents',
    reconnecting: 'Reconnecting...',
    disconnect: 'Disconnect',
  },
  monitor_panel: {
    title: 'System Monitor',
    cpu: 'CPU',
    memory: 'Memory',
    swap: 'Swap',
    disk: 'Disk',
    network: 'Network',
    gpu: 'GPU',
    usage: 'Usage',
    vram: 'VRAM',
    cpu_history: 'CPU Trend',
    used: 'Used',
    free: 'Free',
    rx: 'Rx',
    tx: 'Tx',
    load: 'Load',
    total: 'Total',
    unavailable: 'Unable to fetch remote metrics',
    no_session: 'No active session',
    core: 'cores',
    refresh: 'Refresh',
  },
  status: {
    tabs_count: '{count} tab(s)',
    local: 'local',
    connected: 'connected',
    connecting: 'connecting',
    disconnected: 'disconnected',
    type: 'Type',
    host: 'Host',
    port: 'Port',
    user: 'User',
    auth: 'Auth',
    password: 'Password',
    key: 'Key',
    hostname: 'Hostname',
    kernel: 'Kernel',
    shell: 'Shell',
    agent: 'Agent Fwd',
    enabled: 'Yes',
    status: 'Status',
  },
  session_dialog: {
    title_new: 'New Session',
    title_edit: 'Edit Session',
    name: 'Name',
    type: 'Type',
    host: 'Host',
    port: 'Port',
    username: 'Username',
    password: 'Password',
    remember_password: 'Remember Password',
    group: 'Group',
    no_group: 'No Group',
    new_group: 'New Group...',
    new_group_placeholder: 'Enter new group name',
    cancel: 'Cancel',
    save: 'Save',
    create: 'Create',
    local_terminal: 'Local Terminal',
    command: 'Executable',
    command_placeholder: 'e.g. cmd.exe, /bin/bash',
    working_dir: 'Working Directory',
    working_dir_placeholder: 'Leave empty to inherit parent',
    icon: 'Icon',
  },
  sftp: {
    panel_title: 'Files',
    connect: 'Connect',
    disconnect: 'Disconnect',
    go_up: 'Go Up',
    folder_name: 'Folder name',
    ok: 'OK',
    new_file: 'New file',
    actions: 'Actions',
    empty_directory: 'Empty directory',
    title: 'Files - {host}',
    local: 'Local',
    remote: 'Remote',
    upload: 'Upload',
    download: 'Download',
    rename: 'Rename',
    delete: 'Delete',
    mkdir: 'New Folder',
    refresh: 'Refresh',
    name: 'Name',
    size: 'Size',
    modified: 'Modified',
    permissions: 'Permissions',
    bytes: 'B',
    kb: 'KB',
    mb: 'MB',
    gb: 'GB',
    dir: 'Directory',
    file: 'File',
    close: 'Close',
    edit: 'Edit',
    no_files: 'No files',
    connecting: 'Connecting...',
    disconnected: 'Disconnected',
    drop_to_upload: 'Drop files here to upload',
    uploading: 'Uploading...',
    downloading: 'Downloading...',
    upload_done: 'Done',
    upload_failed: 'Failed',
    cancelled: 'Cancelled',
    confirm_delete: 'Delete {name}?',
    rename_title: 'Rename',
    create_title: 'Create {0}',
    enter_name: 'Enter name',
    owner: 'Owner',
    group: 'Group',
    other: 'Other',
  },
  adb_file: {
    not_connected: 'Device not connected. Open an adb session first, then open the file panel.',
  },
  file_fs: {
    local_ready: 'Local file system — open to browse this machine.',
  },
  tunnel: {
    title: 'Port Forwarding',
    create: 'New Tunnel',
    new_btn: 'New',
    cancel_btn: 'Cancel',
    ssh_server: 'SSH Server',
    authentication: 'Authentication',
    password_optional: 'Password (optional)',
    browse: 'Browse',
    forward_type: 'Forward Type',
    local_label: 'Local (-L)',
    remote_label: 'Remote (-R)',
    dynamic_label: 'Dynamic (-D, SOCKS5)',
    bind_address: 'Bind Address',
    target: 'Target Address',
    create_forward: 'Create Forward',
    private_key_path: 'Private Key Path (optional)',
    local: 'Local',
    remote: 'Remote',
    dynamic: 'Dynamic',
    name: 'Name',
    type: 'Type',
    src_host: 'Source Host',
    src_port: 'Source Port',
    dst_host: 'Dest Host',
    dst_port: 'Dest Port',
    status: 'Status',
    actions: 'Actions',
    start: 'Start',
    stop: 'Stop',
    remove: 'Remove',
    starting: 'Starting',
    running: 'Running',
    stopped: 'Stopped',
    error: 'Error',
    add: 'Add',
    cancel: 'Cancel',
    no_tunnels: 'No tunnels',
    using_session: 'Using current SSH session: {0}',
  },
  proxy: {
    title: 'Proxy Settings',
    add: 'Add Proxy',
    name: 'Name',
    type: 'Type',
    host: 'Host',
    port: 'Port',
    username: 'Username',
    password: 'Password',
    http: 'HTTP',
    socks5: 'SOCKS5',
    jump_host: 'Jump Host',
    save: 'Save',
    delete: 'Delete',
    cancel: 'Cancel',
    private_key_path: 'Private Key Path (optional)',
    select: 'Select',
    no_proxies: 'No proxies',
  },
  snippet: {
    title: 'Quick Commands',
    add: 'Add Command',
    name: 'Name',
    command: 'Command',
    variables: 'Variables',
    send: 'Send',
    edit: 'Edit',
    delete: 'Delete',
    close: 'Close',
    no_snippets: 'No quick commands',
    variable_hint: 'Enter variable value',
    variable_usage_hint: 'Use double braces to wrap variable names, you will be prompted for values when sending',
    cancel: 'Cancel',
    confirm: 'OK',
  },
  trigger: {
    title: 'Triggers',
    add: 'Add Trigger',
    pattern: 'Pattern',
    response: 'Response Command',
    cooldown: 'Cooldown (s)',
    enabled: 'Enabled',
    disabled: 'Disabled',
    edit: 'Edit',
    delete: 'Delete',
    close: 'Close',
    no_triggers: 'No triggers',
    save: 'Save',
    cancel: 'Cancel',
  },
  history: {
    title: 'Command History',
    search: 'Search commands...',
    clear: 'Clear',
    execute: 'Execute',
    delete: 'Delete',
    empty: 'No command history yet. Commands run in the terminal are recorded automatically.',
    no_match: 'No matching commands',
  },
  batch: {
    title: 'Batch Input',
    placeholder: 'Enter command to send to selected terminals...',
    select_all: 'Select All',
  },
  editor: {
    title: 'Edit File',
    save: 'Save',
    saving: 'Saving...',
    saved: 'Saved',
    loading: 'Loading...',
    close: 'Close',
    error: 'Error',
    cut: 'Cut',
    copy: 'Copy',
    paste: 'Paste',
    select_all: 'Select All',
  },
  lock_screen: {
    title: 'Locked',
    placeholder: 'Enter password to unlock',
    unlock: 'Unlock',
    set_password: 'Set Lock Password',
    confirm_password: 'Confirm Password',
    set: 'Set',
    change_password: 'Change Password',
    wrong_password: 'Wrong password',
    password_mismatch: 'Passwords do not match',
    no_password: 'Please set a lock password first',
    lock: 'Lock',
    unlock_failed: 'Unlock failed',
  },
  settings: {
    title: 'Settings',
    language: 'Language',
    language_zh: '中文',
    language_en: 'English',
    theme: 'Theme',
    dark: 'Dark',
    light: 'Light',
    background: 'Background',
    background_image: 'Background Image',
    background_opacity: 'Background Opacity',
    select_image: 'Select Image',
    clear_image: 'Clear Image',
    scrollback: 'Scrollback Lines',
    general: 'General',
    appearance: 'Appearance',
    fullscreen: 'Fullscreen',
    exit_fullscreen: 'Exit Fullscreen',
    guake_mode: 'Guake Mode',
    adb_auto_kill: 'Stop ADB server after last ADB tab closes',
    adb_auto_kill_desc: 'Stops the bundled adb server when the last ADB tab closes; external/system adb is left untouched.',
    shell_context_menu: 'Add “Open in AidTerm” context menu',
    shell_context_menu_desc: 'Adds the command to directories, directory backgrounds, and the desktop background.',
    path_environment: 'Add aidterm to PATH',
    path_environment_desc: 'Allows running aidterm directly from a terminal; new terminals pick it up.',
    single_instance: 'Single instance',
    single_instance_desc: 'Only one AidTerm instance may run; launching again focuses the existing window. Takes effect after restart.',
    lock_screen: 'Enable lock screen',
    lock_screen_desc: 'Shows a lock button in the titlebar; requires a password to unlock when locked.',
    lock_screen_password: 'Lock screen password',
    lock_screen_set_password: 'Set lock screen password',
    lock_screen_change_password: 'Change lock screen password',
    lock_screen_new_password: 'New password',
    lock_screen_confirm_password: 'Confirm new password',
    lock_screen_current_password: 'Current password',
    lock_screen_password_hint: 'No lock screen password set yet. Set one after enabling.',
    lock_screen_wrong_current: 'Current password is incorrect',
    lock_screen_saved: 'Lock screen password saved',
    ai_provider: 'Provider',
    ai_api_config: 'API Configuration',
    ai_status: 'Status',
    ai_configured: '✅ Configured',
    ai_not_configured: '❌ Not configured (enter API Key)',
    ai_clear_history: 'Clear Chat History',
    ai_show: 'Show',
    ai_hide: 'Hide',
  },
  notification: {
    disconnected: 'Connection disconnected',
    reconnected: 'Connection restored',
    error: 'Error',
  },
  keychain: {
    title: 'Key Management',
    generate: 'Generate',
    import: 'Import',
    generate_keypair: 'Generate Key Pair',
    ed25519: 'ED25519 (Recommended)',
    key_name: 'Key Name',
    bits: 'Bits:',
    passphrase: 'Passphrase (optional)',
    key_path: 'Private Key Path',
    browse: 'Browse',
    loading: 'Loading...',
    no_keys: 'No keys yet',
    copy_public: 'Copy Public Key',
    copy_path: 'Copy Private Key Path',
    delete: 'Delete',
    refresh: 'Refresh',
    known_hosts: 'Known Hosts',
    known_hosts_empty: '~/.ssh/known_hosts is empty',
  },
  ai: {
    title: 'AI',
    sidebar_title: 'AI Assistant',
    suggest_command: 'AI suggests command:',
    execute: 'Execute',
    modify: 'Modify',
    cancel: 'Cancel',
    provider: 'Provider',
    api_config: 'API Configuration',
    api_key: 'API Key',
    model: 'Model',
    base_url: 'Base URL',
    status: 'Status',
    configured: '✅ Configured',
    not_configured: '❌ Not configured (enter API Key)',
    clear_history: 'Clear Chat History',
    select_model: 'Select a model',
    explain_selection: '🤖 AI Explain Selection',
    mode: 'Trigger Mode',
    mode_auto: 'Auto',
    mode_prefix: 'Prefix',
    mode_keybinding: 'Keybinding (Ctrl+Enter)',
    prefix: 'Trigger Prefix',
    prefix_desc: 'Lines starting with any of these characters trigger AI response; each character is a separate trigger prefix',
    input_placeholder: 'Ask AI... (Enter to send)',
    cancel_conversation: 'Cancel this Q&A',
    thinking: 'Thinking...',
    executing: 'Executing...',
    analyzing: 'AI analyzing...',
    output_truncated: 'Output truncated for AI',
    config_placeholder: 'Configure the AI service first',
    config_click: 'Click here to open settings',
    no_config: 'Configure API Key in Settings → AI first',
    no_active_terminal: 'No active terminal, please select one',
    reset: 'Reset Chat',
    new_conversation: 'New Chat',
    dangerous_command: 'Dangerous',
    safe_command: 'Safe',
    auto_executing: 'Auto-executing...',
    auto_execute_done: 'Done',
    auto_execute_section: 'Command Execution',
    auto_execute: 'Auto-execute safe commands',
    auto_execute_desc: 'When enabled, AI-classified safe read-only commands execute automatically; dangerous commands always require confirmation',
    command_output: 'Output',
    stop_waiting: 'Stop waiting',
    copy: 'Copy',
    select_all: 'Select All',
    me: 'Me',
    aid: 'Aid',
  },
  common: {
    name: 'Name',
    type: 'Type',
    host: 'Host',
    port: 'Port',
    username: 'Username',
    password: 'Password',
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    edit: 'Edit',
    status: 'Status',
    loading: 'Loading...',
  },
  terminal: {
    connecting: 'Connecting...',
    disconnected: 'Connection lost',
    reconnect: 'Reconnect',
    enter_to_reconnect: 'Press Enter to reconnect',
  },
  about: {
    title: 'About',
    description: 'A cross-platform terminal emulator built with Tauri and Vue, supporting SSH, Telnet, Serial, SFTP and AI assistant.',
    github: 'GitHub',
    check_update: 'Check for Updates',
    checking: 'Checking...',
    up_to_date: 'You are up to date (v{version})',
    update_available: 'New version v{version} available',
    published: 'Released',
    installer: 'Installer type',
    download_install: 'Download & Install',
    downloading: 'Downloading',
    install_restart: 'Install & Restart',
    view_release: 'View Release',
    view_update: 'View Update',
  },
}
